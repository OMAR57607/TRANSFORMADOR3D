 ...
 * con nuestro motor de renderizado ThreeJsVehicleRenderer, simulando el
 * flujo de subida y procesamiento 3D local.
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Elementos del Modo de Visualización
  ...
  // Cargar token de API de Replicate guardado en localStorage
  ...
  function handleFileSelection(file) {
    ...
    addLog(`Captura cargada en memoria: ${file.name}. Listo para procesar.`, 'system');
  }

  // --- EJECUCIÓN DEL FLUJO DE ESCANEO COMPLETO ---
  btnStart.addEventListener('click', async () => {
    const vehicleName = vehicleSelect.options[vehicleSelect.selectedIndex].text;
    const file = fileInput.files[0];

    if (!file) {
      addLog('Error: Selecciona o arrastra un archivo antes de comenzar.', 'error');
      return;
    }
    ...
    // SI EL ARCHIVO ES UN PAQUETE ZIP O VIDEO, SE ENVÍA AL PIPELINE DE RECONSTRUCCIÓN REAL
    btnStart.setAttribute('disabled', 'true');
    btnBrowse.setAttribute('disabled', 'true');
    progressContainer.style.display = 'block';
  ...
  function setMode(mode) {
    ...
    if (mode === 'ia') {
      ...
      if (activeModelUrl) {
        ...
        addLog('No hay ningún escaneo completado cargado. Sube una foto para generar tu malla.', 'warning');
      }
    } else {
  ...
  // Cargar historial de escaneos desde el servidor
  async function loadHistory() {
    try {
      ...
      addLog(`Error al cargar historial: ${err.message || err}`, 'error');
    }
  }
